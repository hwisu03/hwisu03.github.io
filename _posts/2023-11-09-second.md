...
분류, 회귀, 클러스터링, 차원 축소 등을 포함한 광범위한 머신러닝 알고리즘을 제공하고,<br>
예제와 사용 설명서가 잘 되어있어 참고하여 코드를 작성하기 쉽다.<br>
...
## 2-1. 스케일링(scaling)
서로 다른 변수(feature)의 값 범위를 선형변환을 통하여 일정한 수준으로 맞추는 작업<br>
독립변수(feature)별로 값의 변위가 상이하면 종속변수(target)에 대한 영향이 독립변수의 변위에 따라 크게 달라짐 → 머신러닝 시 학습효과가 떨어짐<br>
다차원의 값들을 동일한 수준에서 비교 분석하기 용이하게 만들어 줌<br>
컴퓨터의 비트수로 인하여 다른 값으로 인식되는 오버플로우(overflow)나 언더플로우(underflow)를 방지<br>
최적화 과정에서의 안정성 및 수렴 속도를 향상<br>
특히 k-means 등 거리 기반의 모델에서는 스케일링이 매우 중요<br>
표준화, 정규화, 변환이 있음<br>
...
출처: https://chriver58.quarto.pub/03_PreProcessing/Preprocessing_07_%EC%8A%A4%EC%BC%80%EC%9D%BC%EB%A7%81.html <br>
...
-훈련 데이터는 fit()과 transform() 모두 적용<br>
-평가 데이터는 fit()은 필요없으므로 transform()만 적용해야 함<br>
